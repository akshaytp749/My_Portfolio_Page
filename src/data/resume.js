...
export const systems = [
  ...
  },
  {
    ...
    diagram: {
      nodes: [
        { id: "docs", label: "docs", x: 0, y: 60, why: "The company's sole company-wide RAG — HR, IT, OTC and RC Support corpora, 100k+ vectors in production." },
...
export const stackLayers = [
  ...
  },
  {
    ...
    items: ["Golden-dataset evals", "MLflow", "Langfuse", "Trace monitoring"],
  },
  {
    layer: "Cloud & infra",
...
- Multi-Agent Platform: architected with LangGraph + FastAPI; Dynamic Schema Registry in PostgreSQL decouples agent definitions from code, so new agents register instantly without redeployment (cut onboarding time ~90%); routing layer instrumented with MLflow for run tracking and observability; real-time token streaming over SSE; conversation history persisted in BigQuery for multi-turn context and compliance.
- Production RAG system: the company's sole company-wide RAG system, the authoritative knowledge source across four business domains (HR, IT, OTC, RC Support); Pinecone hybrid retrieval - dense embeddings (Google's Gemini embedding model) + sparse vectors (pinecone-sparse-english-v0); heading-aware chunking with breadcrumb prefixes; domain namespace isolation; metadata-driven region filtering with global fallback; incremental re-ingestion skips unchanged content; a golden-dataset evaluation harness measures 85-95% accuracy, tracked via MLflow; deployed on Google Cloud Run with streaming answers; 100k+ enterprise vectors.
...
- Document Analytics agent: Gemini via Vertex AI with a local-execution architecture to handle datasets beyond context limits; regex-based sanitization layer validates LLM-generated Pandas code before execution - 100% calculation accuracy, no hallucinated numbers.
- NetSuite AI agent: adopted by the full procurement team for daily natural-language querying of procurement data; hybrid model routing (Gemini Flash-lite for intent, Flash for generation); PostgreSQL-backed session management across context switches.
...
- Side projects: "The Interval" (A2A-MCP-World) - persistent multi-agent world where AI agents portraying historical figures live in a cafe, converse over the A2A protocol, and act only through validated MCP tools; every LLM call is traced with Langfuse, a security-focused admission gate runs schema and prompt-injection-refusal checks before any new agent joins, and it is backed by a 118-test suite, all running on free-tier infrastructure. LLM Laptop Compatibility Checker (scans CPU/RAM/VRAM, shows which local LLMs will run, Ollama integration); fully-local AI voice assistant with zero-shot voice cloning (Ollama + F5-TTS + Whisper).
- Evaluation & observability: Akshay builds evals and monitoring into his systems, not as an afterthought - golden-dataset evaluation harnesses (the RAG system is measured at 85-95% accuracy), MLflow for run tracking on the multi-agent platform and RAG, and Langfuse tracing on The Interval. If asked whether he does evals or observability, the answer is yes, with these concrete examples.
...
- Refer to Gemini generically ("Gemini", "Gemini Flash", "Gemini Flash-lite", "Google's Gemini embedding model"). Never state a Gemini version number or a dated model id — not from these facts and not from your own training. Akshay tracks the current Google models, so a specific version you recall is likely already out of date. If pressed for an exact version, say the specific model versions move quickly and suggest emailing him.
