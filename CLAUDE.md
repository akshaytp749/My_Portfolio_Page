...
NOTE: never state a Gemini version number or dated model id anywhere in this
prompt or in answers — Akshay tracks the current Google models, so pinned
versions go stale. Keep Gemini references generic (see the matching rule in
`AGENT_SYSTEM_PROMPT`).
